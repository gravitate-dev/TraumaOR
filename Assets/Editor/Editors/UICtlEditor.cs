...
public class UICtlEditor : EditorWindow, IGUIHelper, IGUIScriptSelector
{
	...
	[MenuItem("Window/UI Control Editor")]
...
//#endif
		...
		if (control is AutoSpriteControlBase)
			ShowSpriteSettings();
		else
			transitions = ((ControlBase)control).GetTransitions(curState);
		...
		if (!Application.isPlaying)
		{
			...
			//-----------------------------------------
			if (control is AutoSpriteControlBase && stateInfo.stateLabel != null)
			{
				BeginMonitorChanges();
				DoStateLabel();
				EndMonitorChanges();
	...
	void DoStateLabel()
	{
		...
		((AutoSpriteControlBase)control).SetStateLabel(curState, EditorGUILayout.TextField(stateInfo.stateLabel, GUILayout.Width(250f)));

		GUI.contentColor = oldColor;
#else
		((AutoSpriteControlBase)control).SetStateLabel(curState, EditorGUILayout.TextField("Label:", stateInfo.stateLabel, GUILayout.Width(250f)));
		GUILayout.FlexibleSpace();
#endif
